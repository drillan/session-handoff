...
裁定は保留（人間の判断待ち）。選択肢は「配列順を優先順位として明示的に並べ替える」
「予算を上げる」「予算を超えたら要約せず先頭 N 行だけ入れる」など。
なお `tests/test_headings_contract.sh` は `INJECT_SECTIONS` を `LC_ALL=C sort` して
比較しているので、並べ替えても契約テストは壊れない。

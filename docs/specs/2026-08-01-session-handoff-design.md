...
並び、どれが最新の続きか判別できなくなる。項目があれば連鎖を 1 件に畳める。
...
2. 各ファイルの frontmatter の `updated`・`continues_from` と `## いま何をしているか` の
   先頭 1 行を読み、候補一覧を人間に提示する。`latest` に記録された session_id を先頭に置く。
   `continues_from` で連なるファイル群は 1 件に畳み、最新のものだけを候補として出す
   （畳んだ件数は併記する）
...
ただし**境界を示す文は字数制限に優先する**。`/handoff` の description は 286 字あり
上限を超えるが、超過分は次の 2 文で、どちらも誤起動を直接防いでいる:

- 「読み込んで再開する側は handoff-load が担当する」— 「前の作業の続きをやりたい」を弾く
- 「揮発性の作業状況であって、会話を超えて残す恒久知見の記憶ではない」— 「この知見を
  覚えておいて」を弾く。§9 の auto-memory / chiso との境界そのもの

字数に収めるためにこの 2 文を削ると、隣接するスキルとの取り違えを description から
判定できなくなる。上限は簡潔さのための目安であって、弁別性より優先しない。
...
- frontmatter（`updated_by: handoff-skill`）、8 セクション、`latest` がいずれも
  §5 の形式どおり生成された

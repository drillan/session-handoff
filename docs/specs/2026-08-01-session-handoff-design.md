...
- 日付: 2026-08-01
- 状態: 実装済み（2026-08-01）。ただし §11 の未検証事項は残っている
...
含めていたが実測しておらず、いったん §11 へ移した。2026-08-01 に実測して展開されることを
確認し、§11.1 の「SKILL.md 本文でのトークン展開」へ移した。skill が Bash 経由で見る環境は
これとは別で、§11.1 の「skill から見た環境」を参照（そちらでは未設定）。
...
**`latest` の意味は「最後に書き込まれた引き継ぎ」であり、意味情報の有無を問わない。**
2 つの書き手は違う出来事を記録している。`pre-compact.sh` が書くのは「最後に compact が
走ったセッション」、`/handoff` が書くのは「最後に意味情報を書いたセッション」。
したがって、セッション A が `/handoff` を実行したあとに、セッション B が `/handoff` を
実行しないまま compact を起こすと、`latest` は中身の無い B のスタブを指す。

これを避けるために `pre-compact.sh` に条件を付けることはしない。フックが
「意味情報のあるファイルなら上書きしない」といった判断を持つと、`latest` は
「最後の書き込み」でも「最後の意味情報」でもない第三の何かになり、
どちらの書き手から見ても予測できなくなる。代わりに `/handoff-load` が候補一覧で
`updated_by` を併記し、人間が中身の有無を見て選べるようにする（8.6 節）。
...
両 SKILL.md にその旨を明記する。`tests/test_headings_contract.sh` が、
`skills/handoff/SKILL.md` の 9 見出しと `session-start.sh` の 2 配列を
実ファイル同士で突き合わせて守る。他のテストは fixture を手書きしているので、
SKILL.md 側だけ言い換えても緑のまま通ってしまう。

### 5.1.2 `## 自動追記ログ` の見出しは常に存在させる

`## 注意` の後に `---` 区切りと `## 自動追記ログ` 見出しを必ず置く。中身が空でも置く。

見出しが無いと、`pre-compact.sh` がファイル末尾へ追記した git の状態が
`## 注意` セクションの内側に入る。`section()` は `^## ` で切るので、
git のノイズが「注意」として compact 直後のコンテキストに注入される。
`/handoff` が新規ファイルを作るときも同じ（8.2 節）。
...
`command` に単一のシェル文字列を書く。当初は `args` 配列形式を予定していたが、
実在するプラグイン 3 つがいずれも単一文字列で、`args` 形式は検証できていない（11.1 節）。
`${CLAUDE_PLUGIN_ROOT}` は二重引用符で囲む。単一引用符では展開されないことを実測している。
...
4. `<session_id>.md` が存在しなければ新規作成し、**`## いま何をしているか` 見出しの下に**
   警告を書く。
   ...
   **冒頭の素の本文に置いてはならない。** 6.3 節は名前で指定した 3 見出しの中身しか
   抽出しないので、見出しの外に置いた警告は compact 直後のコンテキストへ一切届かない。
   「意味情報が無い」という最も重要な信号が黙って落ちる。
   実装当初これを素の本文に置いており、レビューで発覚して修正した
...
**注入対象 3 つの並びは優先順位であり、§5.1 の定義順ではない。**
貪欲に詰める以上、逼迫時は必ず後ろの要素から落ちる。したがって並びがそのまま
「何を最初に捨てるか」を決める。定義順のまま置くと「試して駄目だったこと」が
真っ先に落ちるが、それは §5.3 がこのファイルを作る第一の理由として名指しした
セクションであり、compact の要約が最も落としやすい情報でもある。
compact 時に捨てる対象としては最悪の選択なので、優先順位を上表のとおり明示する。
実装の `INJECT_SECTIONS`（`session-start.sh`）の配列順がこの優先順位そのもので、
`tests/test_session_start.sh` のケース 9 が回帰として固定している。

副作用として、注入本文は「試して駄目だったこと」から始まる。読む順序としては
「いま何をしているか」が先のほうが自然だが、優先順位と表示順を分けると
実装に順序を 2 つ持たせることになる。全セクションが収まる通常時は 3 つとも入り、
落ちるのは逼迫時だけなので、表示順の不自然さより取りこぼしの回避を採る。
この裁定は 2026-08-01 の実測（§11.1）を受けたもの。
...
- 値が取得できない項目は空欄にせず、取得できなかった事実を書く
  （`not a git repository`、`⚠ /handoff を実行しないまま compact が発火した`、
  経過時間の `不明` など）
- **算出できなかった値の代わりに、それらしい数字を出さない。** `elapsed_ja` は
  `date` が解釈できない `updated` や未来の時刻に対して失敗を返し、`session-start.sh` は
  `不明` と表示する。実装当初は `date` の失敗で空文字が入り `$((now - then))` が
  `now` 全体を差分と解釈して「20666日前」を返していた。exit code は 0 のままで、
  確信ありげな嘘がコンテキストに入っていた。この形の欠陥が最も危険で、
  「もっともらしい値で空欄を埋めない」が名指ししているのはこれ
- ただし鮮度が算出できないことを理由に注入自体を諦めない。3 セクションを捨てるほうが
  損失が大きい。取得できなかった事実を書いて注入は続ける
...
2. 各ファイルの frontmatter の `updated`・`continues_from` と `## いま何をしているか` の
   ...
   **自分の子孫のいずれかより `updated` が新しい非葉**（開いたままのセッションが
   後継の作成後も自分のファイルを更新している状態）。比較相手は自分の子孫であって
   「葉のうち最新のもの」ではない。枝分かれしていると葉が複数あり、どれと比べるかで
   結論が変わる。`updated` が最大のものを選ぶのでもなく、葉を基準にそれより新しい
   非葉を足す。
...
SKILL.md 本文でのトークン展開は 2026-08-01 に実測して決着した（§11.1）。
...
この観測は `${CLAUDE_PLUGIN_DATA}` が SKILL.md 本文で展開されたことの**実測ではない**。
モデルが `~/.claude/plugins/data/` を自分で列挙して id を見つけた経路を排除できないため。
決着は下記「SKILL.md 本文でのトークン展開」で別途つけた。

#### SKILL.md 本文でのトークン展開（2026-08-01 21:49）

`/handoff` をこのセッションで実行し、**読み込まれた SKILL.md の本文を直接観測した**。
本文中の `${CLAUDE_PLUGIN_DATA}` は
`/home/driller/.claude/plugins/data/session-handoff-skills-dir` に置換された状態で届いた。
モデルがパスを自力で推測する余地が無い形で観測できたので、これが決着。

- **SKILL.md 本文のトークンは skill 読み込み時に展開される。**
  Bash ツールの環境変数としては未設定（上表）だが、本文経由なら skill はパスを得られる。
  §5・§8 が本文にトークンを書いてパスを示している設計はこれで裏付けられた

同時に、**展開されることによって壊れる書き方**を実測で踏んだ。当時の
`skills/handoff/SKILL.md` の停止条件 3 番目は「`${CLAUDE_PLUGIN_DATA}` が展開されておらず
`${CLAUDE_PLUGIN_DATA}` という文字列のまま見えているなら止まれ」と書いてあり、
条件文の中のトークンも一緒に展開された結果、読み手には
「`<絶対パス>` が展開されておらず `<絶対パス>` のまま見えている」という、
**展開が働いている環境では決して成立しない自己矛盾した条件**として届いた。
守りたい状況でだけ機能しないガードになっていた。

規則として残す: **展開されないことを検出する条件文に、そのトークン自体を書いてはならない。**
展開後の性質（`/` で始まる絶対パスか、`$` や `{` が残っていないか）で判定する。
両 SKILL.md をこの形に直した。静的な読みでは検出できない欠陥で、
レビュー 3 回を通り抜けている。実際に skill を起動して初めて露出した。
...
#### compact プロファイルの実注入（2026-08-01 21:56）

意味情報が入った引き継ぎ（`updated_by: handoff-skill`、8 セクション実記述）で `/compact` を
起こし、`session-start.sh` の注入を実物で観測した。現れた形:

```text
SessionStart:compact hook success: [session-handoff] 引き継ぎあり（最終更新: 4分前 / by handoff-skill）
...
（予算超過のため省略: 試して駄目だったこと）

全文: /home/driller/.claude/plugins/data/session-handoff-skills-dir/-home-driller-repo-session-handoff/0322f6d5-....md
（背景・目的／完了したこと／決定と根拠／恒久知見の候補／注意／自動追記ログ は上記ファイルを Read すること）
```

成立したこと: 見出し行の書式、`elapsed_ja`（`4分前`）、実在セクションだけを案内する
`defer_present` の絞り込み、省略通知、パス案内。

**成立しなかったこと: 注入対象 3 セクションのうち 1 つが落ちた。しかも落ちたのは
「試して駄目だったこと」で、§5.3 がこのファイルを作る第一の理由として名指ししている
セクションそのもの。** 1500 文字の予算に対して意味情報の入った 2 セクションで使い切った。

原因は予算そのものではなく**貪欲な詰め方と配列順の組み合わせ**にある。
`INJECT_SECTIONS` は §5.1 の定義順（いま何をしているか → 未完了・次の一手 →
試して駄目だったこと）で、逼迫時は常に最後の要素から落ちる。
定義順は「読む順序」であって「優先順位」ではないのに、実装では後者として働いている。
§6.4 は貪欲であることは定めているが、**3 セクション間の優先順位を定めていない**。
この欠落が、予算逼迫時に最も高価な情報から捨てる挙動を生んだ。

裁定（2026-08-01・人間の判断）: **配列順を優先順位として明示的に並べ替える。**
予算 1500 文字は据え置き。`INJECT_SECTIONS` を
`("試して駄目だったこと" "いま何をしているか" "未完了・次の一手")` とし、§6.4 に
「この並びは優先順位であって定義順ではない」と明記した。
併せて `tests/test_session_start.sh` にケース 9（予算が 1 セクション分しかないとき
残るのは「試して駄目だったこと」）を追加し、定義順への差し戻しを回帰で止める。
既存ケース 3 も、巨大なセクションを新しい並びの 2 番目へ移した。
最後に置いたままでは「落ちたセクションより後ろの短いセクションが残る」という
貪欲さの検証が空振りするため（テストが通っても何も確かめていない状態になる）。

`tests/test_headings_contract.sh` は `INJECT_SECTIONS` を `LC_ALL=C sort` して
比較しているので、この並べ替えで契約テストは壊れない。

...
上限は stdout 出力全体で 2500 文字（全角・半角を区別せず 1 文字として数える。`wc -m` 基準）。
...
**並べ替え後の再測定（同日）: それでも 3 セクションは入らない。**
同じ引き継ぎファイルに対して `session-start.sh` を直接実行した結果:

```text
[session-handoff] 引き継ぎあり（最終更新: 13分前 / by handoff-skill）
## 試して駄目だったこと
## いま何をしているか
（予算超過のため省略: 未完了・次の一手）
全文: …
...
出力は 1270 文字。実測したセクション長は「試して駄目だったこと」885 文字、
「いま何をしているか」106 文字、「未完了・次の一手」689 文字で、
本文だけで 1686 文字（区切りの空行込み）。header・footer・省略通知の予約を足すと
約 2030 文字となり、**1500 文字の予算では原理的に 3 つとも入らない。**

つまり並べ替えは「何を失うか」を変えただけで「1 つ失う」ことは変えていない。
今度落ちるのは「未完了・次の一手」で、§5.1 が「次に手を動かす人が最初に読む場所」と
定めたセクション。優先順位の明示は必要な修正だったが、**十分ではなかった**。
この引き継ぎファイルは肥大した特異例ではなく、実装が一段落した通常の規模である点に注意。

**予算の裁定（2026-08-01・人間の判断）: `MAX_CHARS` を 1500 から 2500 へ引き上げる。**
実測値 2030 に約 470 文字の余裕を持たせた値。上の「予算 1500 文字は据え置き」という
先の裁定は、この再測定によって覆した。

引き上げ後に同じファイルで三度目の測定をした結果、**3 セクションとも入り、
省略通知は出なくなった**（出力 1936 文字）。§11 の「compact 注入の実確認」はこれで閉じる。

```text
[session-handoff] 引き継ぎあり（最終更新: 16分前 / by handoff-skill）
## 試して駄目だったこと
## いま何をしているか
## 未完了・次の一手
全文: …
...
予算を上げても優先順位の明示は無駄にならない。2500 を超えるほど引き継ぎが育てば
また落ち始めるので、そのときに何から捨てるかは依然として決まっている必要がある。

テスト側の fixture は予算に対して相対的に効くため、`MAX_CHARS` を変えるときは
`tests/test_session_start.sh` のケース 6（`big1`/`big2`）とケース 9（`fill`）を
併せて見直す。特にケース 9 は各セクションが「予算の半分より大きく予算以下」でないと
2 つ入ってしまい、通っても何も検証しない状態になる。

...
# 当初 1500 だったが、実装が一段落した通常規模の引き継ぎで実測したところ、
# 注入対象 3 セクションの本文だけで 1686 文字あり、header・footer・省略通知を足すと
# 約 2030 文字だった。1500 では原理的に 1 つ落ちる（2026-08-01 の実測。§11.1）。
# 2500 は実測値に約 470 文字の余裕を持たせた値。これを超えれば従来どおり
# INJECT_SECTIONS の優先順位に従って後ろから落ちる。
MAX_CHARS=2500

# compact プロファイルで注入する見出し（§6.4）。
# **この並びは優先順位であって §5.1 の定義順ではない。** 詰め方が貪欲なので、
# 予算が逼迫すると常に後ろの要素から落ちる。定義順のまま置くと、失うと最も高くつく
# 「試して駄目だったこと」が真っ先に捨てられる（2026-08-01 に実測で踏んだ。§11.1）。
# それはこのプラグインが存在する理由そのものを compact 時に手放すことになる。
INJECT_SECTIONS=("試して駄目だったこと" "いま何をしているか" "未完了・次の一手")
...
# 通知コストを差し引くと、通知を書き足した結果 MAX_CHARS を超える恐れがある。
